...
The make-money search is concluded (capstone thesis `docs/research/0017-portfolio-thesis.md`: ten
studies, one apparatus, one qualified pass). The Phase 0 paper-trading system for Study 6 is built
(`src/riskpremia/live/` + the deployment runbook), reusing the frozen rule as the single source of
truth, so a zero-capital out-of-sample track record can accrue now. Sam is paper-trading first (no
IRA / small sum yet); real money waits for a tax-advantaged account.
**Next step: run the monthly paper loop to start the track record, and when Sam wants broker-side
fills, add the Alpaca-paper executor (the runbook section 11/12). Or open a fresh research fork.**
...
& $py -m scripts.build_live_levels  # one-time/network: fetch VTI/IEF/SGOV month-end adjusted closes -> committed live seed
& $py -m scripts.live_signal  # no-network: print this month's Study 6 long-or-cash target from the committed levels
& $py -m scripts.paper_rebalance  # no-network: run one month's paper rebalance (live_state/, gitignored) of the Study 6 rule
