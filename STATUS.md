...
Last updated: 2026-06-07 (session 15: Study 8 SELECTED and PRE-REGISTERED. After the Study 7 merge (PR #25), a four-lens fork plus an adversarial cross-check selected a VOLATILITY-MANAGED MARKET-PORTFOLIO study (a deployable swing, the ADR 0008 registered backup), scoped per the quant lens to the managed market scored managed-minus-unmanaged and net of the scaling-turnover and financing costs, with the literature's predicted market-survives / factors-die asymmetry pre-registered as a secondary. ADR 0010 + docs/research/0011 freeze the signal (Moreira-Muir inverse-variance with the c-normalization), the leverage cap (2.0x via a leveraged ETF), the cost model, and the gate (Study 6's no-fit full-sample conditional PSR(0) primary). Both feasibility gates pass pre-cleared (data already committed for Study 6; leveraged-ETF-or-cash implementation cannot destroy a small account). Industry/sector momentum is the registered backup. Build follows. PRIOR session 14: Study 7 (crypto funding dispersion) BUILT + measured + shipped (PR #25 merged).).

**Study 8 (volatility-managed market portfolio, ADR 0010): SELECTED AND PRE-REGISTERED; build pending.** A deployable swing (the ADR 0008 registered backup), chosen by a four-lens fork + adversarial cross-check over industry/sector momentum. It adjudicates the contested volatility-managed claim (Moreira-Muir 2017 positive vs Cederburg et al. 2020 / Barroso-Detzel 2020 / DeMiguel et al. 2024 negative-OOS-net-of-cost) on the project's deflated net-of-cost gate. Scoped (per the quant lens) to the managed MARKET portfolio (the lone documented cost-survivor), scored as a direct managed-minus-unmanaged test, so the result is defensible regardless of sign; the literature's predicted market-survives / factors-die ASYMMETRY is pre-registered as a secondary. FROZEN METHOD (ADR 0010): signal = Moreira-Muir inverse-variance scale `w_m = c * sigma_target^2 / RV_{m-1}` with `RV` the previous calendar month's sum of squared daily excess returns (a single estimator, no grid) and `c` the full-sample normalization making managed vol = unmanaged vol (removes the free-leverage confound); leverage cap 2.0x via a leveraged equity ETF or cash (NOT margin, so no margin call, position cannot go below zero); cost charged on the CONTINUOUS monthly weight change + financing on the levered leg (this is where Barroso-Detzel kills the factor versions, so it is modeled before freezing); gate mirrors Study 6's no-fit primary (full-sample conditional PSR(0) of the managed market's net excess, deflated by the frozen trial count; CPCV worst fold + 2022+ recency as stress); adjudication statistic = managed-minus-unmanaged conditional PSR(0). Secondary applies the same scaler to SMB/HML/RMW/CMA/WML (same French library) for the asymmetry. Guardrails: framed as an ADJUDICATION not a search, no DoF grid, cost must hit scaling turnover + financing, the redundancy-with-Study-6 concern answered (vol-timing is near-orthogonal to the price trend; the managed-vs-Study-6 correlation is reported). Gate 1 (data) PASS pre-cleared (French daily Mkt-RF + RF already committed for Study 6; secondary factors same source/loader family); Gate 2 (stress) PASS (leveraged-ETF-or-cash, no short, no margin cascade). Considered-and-deferred: industry/sector momentum (the registered backup; not now because of its forking-path surface + larger build + settled-classic narrative); a momentum+trend overlay (higher DoF, rhymes with Studies 3/4/6); managed-factors-only (a guaranteed null). Detail in `docs/research/0011-volatility-managed-equity-design.md`. Next: build the gate from the committed French fixture, run it, ship the verdict + figures.
...
with figures. Study 8 (volatility-managed market portfolio, ADR 0010) is selected and
pre-registered: a deployable swing that adjudicates the contested volatility-managed claim on the
project's deflated net-of-cost gate, scoped to the managed market with the predicted
market-survives / factors-die asymmetry as a secondary, on the clean Kenneth French data already
in the repo.
**Next step: build the Study 8 volatility-managed gate and ship the verdict.**
...
- 278 offline pass + 18 live `network` tests (the figure render tests run
  locally, skipif in CI); mypy strict (src + scripts, 81 source files) / ruff /
